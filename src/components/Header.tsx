import {Plus, X, PencilSimple} from 'phosphor-react'
import * as Dialog from '@radix-ui/react-dialog'

import LogoImage from '../assets/logo.svg'
import { NewHabitForm } from './NewHabitForm'
import { HabitsVizualization } from './HabitsVizualization'
            ...
            <div className='flex gap-5'>
                ...
                <Dialog.Root>
                    <Dialog.Trigger 
                        ...
                        className='border border-violet-500 font-semibold rounded-lg px-6 py-4 flex items-center gap-3 hover:border-violet-300 transition-colors focus:outline-none focus:ring-2 focus:ring-violet-700 focus:ring-offset-2 focus:ring-offset-background'
                    >
                        <PencilSimple size={20} className="text-violet-500"/>
                            Lista de hábitos
                    </Dialog.Trigger>
                    <Dialog.Portal className='overflow-hidden'>
                        <Dialog.Overlay className='w-screen h-screen bg-black/80 fixed inset-0' />

                        <Dialog.Content className='absolute p-10 bg-zinc-900 rounded-2xl w-full max-w-3xl top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 overflow-y-scroll max-h-[90%] scrollbar scrollbar-thumb-gray-700 scrollbar-track-zinc-800/0  scrollbar-thumb-rounded-lg'>
